...
Board: ESP32C3 Dev Module or ESP32C3 Dev Module
CDC On Boot: Enabled (if you want a serial monitor connection via the USB port)
Partition Scheme: default 4Mb with SPIFFS (1.2MB App) or Minimal SPIFFS (1.9 MB app)
                  OTA is required 
-------------------------------------------------------------------------------------
...
const int NUM_LEDS = 24;         // How many leds in fibonacci clock?
                                 // If Not 12 LEDs --->  Update also LED positions in  setPixel() (at the end of file) TOO!  
...
Adafruit_NeoPixel LEDstrip = Adafruit_NeoPixel(NUM_LEDS, LED_PIN, NEO_GRB + NEO_KHZ800);  //NEO_RGB NEO_GRB
                            #endif  //LED2812
                                   #ifdef ESP32S3_ZERO
Adafruit_NeoPixel PCB_LED = Adafruit_NeoPixel(1, 21, NEO_RGB + NEO_KHZ800);  //NEO_RGB NEO_GRB                                          // If LED is connected to Pin IO3 with 470 ohm resistor
                                   #endif // ESP32S3_ZERO
...
void EverySecondCheck(void)
{
 ...
 if (msLeap >999)                                                                             // Every second enter the loop
 {
  ...
  if (Mem.StatusLEDOnOff)  SetStatusLED(0,WIFIConnected*255,deviceConnected*255);  
...
//                Tekstprintln(sptext);
 ...
 //            Tekstprintln(sptext); 
    ...
    case 'Q':   
             ...
             Tekstprintln("  Q1= Mondriian2");
             Tekstprintln("  Q2= RGB");
             Tekstprintln("  Q3= Greens");
             Tekstprintln("  Q4= Pastel");
             Tekstprintln("  Q5= Modernr");
             Tekstprintln("  Q6= Cold");
             Tekstprintln("  Q7= Warm");
             Tekstprintln("  Q8= Earth");
             Tekstprint(  "  Q9= Mondriaan3");
             sptext[0]=0;
               }
             if (InputString.length() == 3 )
               {
